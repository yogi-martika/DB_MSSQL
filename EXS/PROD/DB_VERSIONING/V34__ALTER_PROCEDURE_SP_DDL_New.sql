...
BEGIN
	...
	(
	  SELECT ROW_NUMBER() OVER (ORDER BY [ID]) AS nofile,
				...
				CASE WHEN EventType='CREATE_DATABASE' THEN 'RESTORE DATABASE ['+ DBName +'] FROM  DISK = ''F:\\10.10.10.177\FolderBackupCICD\'+ DBName +'_backup_4Flyway.bak'' WITH MOVE '''+ DBName +''' TO ''F:\MDFDB\'+ DBName +'.mdf'',  MOVE '''+ DBName +'_log'' TO ''G:\LDFDB\'+ DBName +'_log.ldf'',  RECOVERY;' ELSE NULL END RestoreScript,
				GETDATE() CreatedDate,
				0 IsExport,
				NULL ExportDate,
				IsGenerate
