...
BEGIN
	...
	(
	  SELECT ROW_NUMBER() OVER (ORDER BY [ID]) AS nofile,
				CASE WHEN EventType='CREATE_DATABASE' THEN 'V'+CAST(ROW_NUMBER() OVER (ORDER BY [ID]) AS VARCHAR(MAX))+'__'+EventType+'_'+DBName+'.sql' ELSE 'V'+CAST(ROW_NUMBER() OVER (ORDER BY [ID]) AS VARCHAR(MAX))+'__'+EventType+'_'+TableName+'.sql' END AS [filename],
				ID,
				DBName,
				ScriptVw,
